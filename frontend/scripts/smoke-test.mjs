const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5640';
...
const adminCookie = await login('admin@carbon-credit-operations.local', 'admin123');
const managerCookie = await login('manager@carbon-credit-operations.local', 'manager123');
const analystCookie = await login('analyst@carbon-credit-operations.local', 'analyst123');
